...
def gravatar_filter(email, size=100, default='retro', rating='g'):
    ...
    # Gravatar requires the email to be lowercase and hashed via MD5
    clean_email = email.strip().lower().encode('utf-8')
    email_hash = hashlib.md5(clean_email).hexdigest()

    # Build the URL with the parameters you previously used
    query_params = urllib.parse.urlencode({'d': default, 's': str(size), 'r': rating})
